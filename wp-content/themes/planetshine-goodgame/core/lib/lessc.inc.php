...
if ( !class_exists('lessc') ) {
	class lessc {
		...
		protected function tryImport($importPath, $parentBlock, $out) {
			...
			$root = $parser->parse(goodgame_wp_file_get_contents($realPath));
		...
		protected function lib_data_uri($value) {
			...
			if($fullpath && ($fsize = filesize($fullpath)) !== false) {
				...
				if($fsize/1024 < 32) {
					...
					//	$url = sprintf('data:%s;base64,%s', $mime, base64_  [some whitespace]  encode(goodgame_wp_file_get_contents($fullpath)));
		...
		public function compileFile($fname, $outFname = null) {
			...
			$out = $this->compile(goodgame_wp_file_get_contents($fname), $fname);

			$this->importDir = $oldImport;

			if ($outFname !== null) {
				return goodgame_wp_file_put_contents($outFname, $out);
			}
