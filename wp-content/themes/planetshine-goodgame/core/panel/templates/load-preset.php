<div class="color-preset-list clearfix">
    <?php
    $editor_url = get_admin_url() . 'customize.php?return=' . get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin&view=setup&section=load_preset'; ;
    ?>
    ...
    <?php
    $presets = goodgame_gs('presets', false);
    $meta = goodgame_gs('presets_meta', false);
    $install_url = get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin&view=setup&section=load_preset';
            ...
            if(!empty($meta[$key]))
            {
                ...
                ?>
                    <div class="preset-item" id="<?php echo esc_attr($key); ?>">
                        <div class="preset-image"><img src="<?php echo GOODGAME_IMG_URL . $item_meta['image']; ?>" alt="preset" /></div>
...
<script type="text/javascript">
    jQuery('.preset-item .button-2').click(function(e){		//option update
        ...
        var nonce = '<?php echo wp_create_nonce('goodgame_load_style_preset') ?>';
        var data = { action: 'goodgame_load_style_preset', _ajax_nonce: nonce, preset: preset};
