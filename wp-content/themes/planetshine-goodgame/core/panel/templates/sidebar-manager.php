<?php
    ...
    $pages_sidebars = goodgame_gs('page_sidebars');

    ?>

        <div class="section-item clearfix" id="all_sidebars">
            <ul class="sidebar-list">
                <h3><?php esc_html_e('All Sidebars', 'planetshine-goodgame'); ?></h3>
                <?php
                    $sidebars = goodgame_get_sidebars();
                    foreach($sidebars as $sidebar)
                    {
        ...
        <div class="section-item clearfix" id="create_sidebar">
            <h3><?php esc_html_e('Create Sidebar', 'planetshine-goodgame'); ?></h3>
            <form name="add-new" class="no-submit">
                <input type="hidden" name="action" value="new" />
                <?php goodgame_output_theme_setting(array(
                    'slug' => 'name',
                    'title' => 'Sidebar name',
                    'type'  => 'textbox',
                    'value' => ''
        ...
        <div class="section-item clearfix" id="manage_sidebars">
            <h3><?php esc_html_e('Manage Sidebars', 'planetshine-goodgame'); ?></h3>
            <form name="manage-sidebars" class="no-submit">
                ...
                foreach($templates as $key => $template)
                {
                    goodgame_output_theme_setting(array(
                        ...
                        'title' => $template,
                        'type'  => 'select',
                        'value' => goodgame_get($pages_sidebars, $key, NULL),
    ...
    <script type="text/javascript">
        jQuery(document).ready(function () {
            ...
            jQuery('#add_new').click(function(){
                ...
                var nonce = '<?php echo wp_create_nonce('goodgame_save_sidebar') ?>';
                var data = { action: 'goodgame_save_sidebar', _ajax_nonce: nonce, data: result};
            ...
            jQuery('.sidebar-list').on('click', '.delete-sidebar', function(){;
                ...
                var admin_ajax = '<?php echo site_url().'/wp-admin/admin-ajax.php'; ?>';
                var nonce = '<?php echo wp_create_nonce('goodgame_save_sidebar') ?>';
                var data = { action: 'goodgame_save_sidebar', _ajax_nonce: nonce, data: data};
