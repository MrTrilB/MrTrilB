    ...
    <form name="settings" class="no-submit">
        ...
        <div class="section-item clearfix banner-form" id="<?php echo esc_attr($ads_item['slug']); ?>">
                ...
                <?php
                    ...
                    {
                        $items = goodgame_get_active_banners($size);
                    ...
                    if(!empty($ads))
                    {
                        ...
                        echo '<div class="form-item clearfix">';
                            echo '<p class="label">' . esc_html__('Check ads for this location', 'planetshine-goodgame') . '<span class="tooltip-1"><i>' . esc_html__('If you select more than one ad, then they will be shown in a rotation', 'planetshine-goodgame') . '</i></span>' . '</p>';
                    ...
                    else
                    {
                        ...
                        echo '<div class="form-item clearfix">';
                            ...
                            echo '<strong>' . esc_html__('There are no active ads for this location! ', 'planetshine-goodgame') . ' ' . esc_html__('Supports: ', 'planetshine-goodgame') . $supports . '</strong>';
                        echo '</div>';
                    }
                ?>
            </div>
...
jQuery(document).ready(function () {
    ...
    jQuery('#save').click(function(){
        ...
        var nonce = '<?php echo wp_create_nonce('goodgame_save_ad_locations') ?>';
        var data = { action: 'goodgame_save_ad_locations', _ajax_nonce: nonce, data: result};
