<div class="section-item clearfix" id="backup_settings">
    <h3><?php esc_html_e('Backup Settings', 'planetshine-goodgame'); ?></h3>
    <?php
    goodgame_output_theme_setting(array(
            'slug'        => 'settings_export',
            'title'       => 'Exported settings',
            'type'        => 'textarea',
            'value'       => GOODGAME_SETTINGS_INSTANCE()->export_settings(),
            'description' => '',
...
<div class="section-item clearfix" id="import_settings">
    <h3><?php esc_html_e('Import Settings', 'planetshine-goodgame'); ?></h3>
    <form name="import-settings" class="no-submit">
        ...
        ?>
        <div class="row">
            <a href="#" id="import-settings" class="button-1"><?php esc_html_e('Import', 'planetshine-goodgame') ?></a>
        </div>
    </form>
</div>

<div class="section-item clearfix" id="reset_settings">
    <h3><?php esc_html_e('Reset Settings', 'planetshine-goodgame'); ?></h3>
    <div class="row">
        <a href="#" id="reset-theme" class="button-1"><?php esc_html_e('Reset Theme', 'planetshine-goodgame') ?></a>
    </div>
</div>
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery('#import-settings').click(function(){
            ...
            var nonce = '<?php echo wp_create_nonce('goodgame_import_settings') ?>';
            var data = { action: 'goodgame_import_settings', _ajax_nonce: nonce, data: result};
        ...
        jQuery('#reset-theme').click(function(){
            ...
            {
                var admin_ajax = '<?php echo site_url().'/wp-admin/admin-ajax.php'; ?>';
                var nonce = '<?php echo wp_create_nonce('goodgame_reset_settings') ?>';
                var data = { action: 'goodgame_reset_settings', _ajax_nonce: nonce};
