<?php
    ...
    {
        $goodgame_demo_list = get_demo_content_demo_list();
    }
    if(function_exists('get_demo_content_demo_list'))
    {
        $goodgame_demo_description = get_demo_content_demo_desc();
    }
	...
	if(class_exists('GoodGame_Demo_Export') && class_exists('GoodGame_Demo_Import'))
	{
		...
		if(!empty($_GET['goodgame_action']))
		{
			if(
				$_GET['goodgame_action'] == 'install-demo'
				&&
				!empty($_GET['demo'])
				&&
				!empty($goodgame_demo_list)
			)
			{
				if(in_array($_GET['demo'], $goodgame_demo_list))
				{
					$settings_url = get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin&view=setup&section=demo_import';
					?>
					...
					<script type="text/javascript">
						jQuery(document).ready(function () {
							...
							function launch_import_step(demo, key, step)
							{
								...
								var nonce = '<?php echo wp_create_nonce('goodgame_demo_import_launcher') ?>';
								var data = { action: 'goodgame_demo_import_launcher', _ajax_nonce: nonce, data: data};
			...
			elseif($_GET['goodgame_action'] == 'rollback-demo')
			{
				$result = GoodGame_Demo_Import::rollbackDemo();
				$settings_url = get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin&view=setup&section=demo_import';
				?>
				<div class="section-item demo-install-report clearfix">
					<?php
			...
			elseif($_GET['goodgame_action'] == 'export')
			{
				/* Export */
				if(isset($_GET['preset']))
				{
					$preset = esc_attr($_GET['preset']);
					GoodGame_Demo_Export::launchExport($preset);
				...
				else
				{
                   ?>No preset provided<?php
                    ...
                    //$export = GoodGame_Demo_Export::exportPages(array('p' => 1730));
					//echo '<textarea>' .  base64_encode(serialize($export)) . '</textarea>';
				}
			}
		}
		else
		{
			$install_url = get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin&view=setup&section=demo_import&goodgame_action=install-demo';
			$rollback_url = get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin&view=setup&section=demo_import&goodgame_action=rollback-demo';
			$current = GoodGame_Demo_Import::getCurrentImport();
			?>
			...
			<?php if(!empty($current) && !empty($goodgame_demo_list) && !empty($goodgame_demo_description)) : ?>
				<div class="section-item current-active-demo clearfix">
					<h3>Current active demo import</h3>
					<?php
						if(!empty($goodgame_demo_description[$current['demo']]))
						{
							$description = $goodgame_demo_description[$current['demo']];
							?>
							<div class="demo-item">
								<div class="demo-image"><img src="<?php echo GOODGAME_IMG_URL . $description['image']; ?>" alt="preset" /></div>
			...
			<div class="demo-import-list clearfix">
				...
				<?php
				if(!empty($goodgame_demo_list) && !empty($goodgame_demo_description))
				{
					$c = 0;
					foreach($goodgame_demo_list as $demo_item)
					{
						if(!empty($goodgame_demo_description[$demo_item]) && (empty($current) || $current['demo'] != $demo_item ))
						{
							?>
							<div class="demo-item">
								<div class="demo-image"><img src="<?php echo GOODGAME_IMG_URL . $goodgame_demo_description[$demo_item]['image']; ?>" alt="preset" /></div>
								<div class="page-description">
									<h3>
										<?php echo esc_html($goodgame_demo_description[$demo_item]['name']); ?>
	...
	{
		echo '<div class="section-item"><h4>' . esc_html__('Please active the "Planetshine GoodGame Theme Extension" plugin to enable the demo import feature', 'planetshine-goodgame') . '</h4></div>';
	}
