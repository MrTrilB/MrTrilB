...
?>
    ...
    <div class="sidebar">
        <div class="logo">
            <img src="<?php echo GOODGAME_ADMIN_ASSET_URL; ?>images/logo-1.png" alt="Planetshine Control Panel" />
        </div>
        <?php
            ...
            if(!empty($head))
            {
                ...
                $section = goodgame_get($_GET, 'section', 'ads_manager');   //get section; defaults to ads_manager

				if($view == 'ads_manager')
				{
					$section = goodgame_get($_GET, 'section', 'ads_manager');   //get section;
				}
				elseif($view == 'setup')
				{
					$section = goodgame_get($_GET, 'section', 'status');
				}
                ...
                foreach($head as $h)
                {
                    ...
                    if($h['type'] == 'goodgame_visual_editor')
                    {
                        echo '<a href="' . esc_url($h['link']) . '"><i class="fa fa-' . $h['icon'] . '"></i>' . $h['name'] . '</a>';
                    }
					else
                    {
                        echo '<a href="' . esc_url( get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin' . '&view=' . $h['slug'] ) . '"><i class="fa fa-' . $h['icon'] . '"></i>' . $h['name'] . '</a>';
                    }
                     ...
                     if(!empty($h['children']))
                     {
                         ...
                         if($has_multiple_levels)
                         {
                            ...
                            {
                                $class = '';
                                $link = get_admin_url() . 'admin.php?page=' . goodgame_gs('theme_slug') . '-admin' . '&view=' . $h['slug'] . '&section=' . $ch['slug'];
                                $class = 'has-children';
