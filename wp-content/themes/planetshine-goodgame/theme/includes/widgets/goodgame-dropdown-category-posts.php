<?php
class GoodGameDropdownCategoryPosts extends WP_Widget {
	...
	function __construct() {
		...
		$this->widget_cssclass = 'goodgame_dropdown_category_posts';
		$this->widget_description = esc_html__( 'Display dynamically switchabe post categories', 'planetshine-goodgame' );
		$this->widget_idbase = 'goodgame_dropdown_category_posts';
		$this->widget_name = esc_html__( 'GoodGame Dropdown Post Categories', 'planetshine-goodgame' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->widget_cssclass, 'description' => $this->widget_description );

		/* Create the widget. */
		parent::__construct('goodgame_dropdown_category_posts', $this->widget_name, $widget_ops);
	...
	function widget($args, $instance)
    {
		...
		$cache = wp_cache_get('goodgame_dropdown_category_posts', 'widget');
		...
		<?php echo $before_widget; ?>
			...
			<div class="sorting">
				<div class="buttons">
					<a href="#<?php echo esc_attr($unique_id); ?>_all" class="btn btn-sort active"><span><?php esc_html_e('All recent articles', 'planetshine-goodgame'); ?></span></a>

					<?php if(!empty($categories)) {
						foreach($categories as $cat)
						{
							$category = get_category_by_slug($cat);
							echo '<a href="#' . esc_attr($unique_id) . '_' . esc_attr($category->slug) . '" class="btn btn-sort"><span>' . $category->name . '</span></a>';
						}
					} ?>
				</div>
			</div>
			...
			<div class="container post-block post-image-90">
				<?php
					...
					foreach($categories as $key => $category)
					{
						...
						$items = goodgame_get_post_collection($params, 6, 1);
		...
		wp_cache_set('goodgame_dropdown_category_posts', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance )
    {
		...
		if ( isset($alloptions['goodgame_dropdown_category_posts']) )
			delete_option('goodgame_dropdown_category_posts');
    ...
    {
		wp_cache_delete('goodgame_dropdown_category_posts', 'widget');
	}
	...
	function form( $instance )
    {
        ...
        ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'cats' )); ?>"><?php esc_html_e( 'Categories:', 'planetshine-goodgame' ); ?></label><br/>
